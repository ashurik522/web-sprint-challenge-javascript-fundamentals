...
//🚀🚀🚀 ⬇️ 📝 Explanation ⬇️ 📝 🚀🚀🚀: 
//The nested function can access anything within the function it is nested in and on the global scope. If const internal was in stead declared in the 
//nested function, it would only be accesible there. 
...
const zooAnimals = [
  ...
  */
  ...
  function lowerCaseNames(zooArray){
    let lowerCase = zooArray.map(function(item){
  ...
  */
  ...
  function lowPopulationAnimals(zooArray){
    let lowPop = zooArray.filter(function(item){
  ...
  */
  ...
  function USApop(zooArray){
    let pop = zooArray.reduce(function(acc, item){
...
// 🦄🦄🦄 Topic 4: Classes 🦄🦄🦄 //
//Using CuboidMakerTwo, take your prototypes from above and refactor into class syntax. Then, create an object called cuboidTwo that uses the 
//new keyword to use our CuboidMakerTwo class.
